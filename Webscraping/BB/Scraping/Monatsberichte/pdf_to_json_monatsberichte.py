...
def extract_pdf_text(downloaded_pdf_file_name):
    print(f'Extract Data from the PDF file:- {downloaded_pdf_file_name}')
    pdf_text_content = ""
    doc = fitz.open(downloaded_pdf_file_name)  
    total_pages = doc.pageCount
    for page in range(total_pages):
        tmp = doc[page]
        text = tmp.getText("text")
...
def not_dl_pdf(p_url):
    ...
    try:
        if '.pdf' in p_url:
            ...
            os.chdir("Monatsberichte_PDFs")
            folder=os.getcwd()

            for item in os.listdir(folder):
                if item==downloaded_pdf_file_name:
                    os.chdir(os.path.dirname(os.getcwd()))
                    return ("Monatsberichte_PDFs/"+downloaded_pdf_file_name)
