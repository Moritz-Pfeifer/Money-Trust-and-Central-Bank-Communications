...
driver.get(
    "https://web.archive.org/web/20161203071308/https:/www.banque-france.fr/la-banque-de-france/communiques-et-discours-des-autorites-de-la-banque/communiques-de-presse.html")

data = []

time.sleep(5)
try:
    data = driver.execute_script("function getElementsByXpath(xpathStr, scope = document) {var allElements = [];"
                                 ...
                                 "return allElements;}"
                                 "var yearWisePRs = [];var singleYearPRs = {};singlePR = {};singleYearPRs = {};"
                                 "singleYearPRs.year = null;"
                                 "singleYearPRs.PRs = [];"
                                 "var years = document.getElementById('menu-tabs').getElementsByTagName('li');"
                                 "for (i = 0; i < years.length; i++) {"
                                 "var yearVal = parseInt(years[i].innerText);"
                                 "if (yearVal > 2014) {"
                                 "console.log('skipped ', yearVal);"
                                 "} else {"
                                 "console.log('processing:', yearVal);"
                                 "var thisYear = yearVal;"
                                 "years[i].getElementsByTagName('a')[0].click();"
                                 ...
                                 "extraDate += ' ' + foobarText;}}"
                                 "extraDate = extraDate.split('undefined').join('');"
                                 "extraDate = extraDate.trim();"
                                 "singlePR.title = extraDate + ' - ' + singlePR.title;}"
                                 "var allAttachments = thisYearsPRs[j].getElementsByTagName('a');"
                                 "singlePR.attachments = [];"
                                 "for (k = 0; k < allAttachments.length; k++) {"
                                 "if (allAttachments.length == 1) {"
                                 "singlePR.attachments.push({"
                                 "href: allAttachments[k].href,"
                                 "title: allAttachments[k].innerText"
                                 "});} else {if (allAttachments[k].innerText.toLowerCase() =="
                                 "'Communiqué de presse'.toLowerCase()) {"
                                 "singlePR.attachments.push({"
                                 "href: allAttachments[k].href,"
                                 "title: allAttachments[k].innerText});"
                                 "} else {console.log('Skipped', allAttachments[k].innerText);}}}"
                                 "singleYearPRs.PRs.push(singlePR);}"
                                 "yearWisePRs.push(singleYearPRs);}}"
                                 "return yearWisePRs;")
    print(data)
    ...
    for year_object in data:
        ...
        for pr_index, pr_object in enumerate(year_object["PRs"]):
            for attachment in pr_object["attachments"]:
                ...
                content_type = None
                print("***********************************************************************************************")
                print(Fore.CYAN + f'Press Release {pr_index + 1}/{len(year_object["PRs"])}')
                ...
                if ext == "pdf":
                    ...
                    if status:
                        item_list.append(
                            ['communique-de-presse', download_url, pr_object["title"], content,
                        ...
                        item_list.append(
                            ['communique-de-presse', attachment["href"], pr_object["title"], 'PR2_FAILED_TO_DOWNLOAD',
                    ...
                    try:
                        ...
                        item_list.append(
                            ['communique-de-presse', attachment["href"], pr_object["title"], content,
                        ...
                        if not fallback_status:
                            ...
                            item_list.append(
                                ['communique-de-presse', attachment["href"], pr_object["title"], 'PR2_FAILED_TO_FETCH',
                                 0, 'Banque de France', file_helper.try_parse_date(pr_object["title"], year_object["year"])])  # ContentType: 0 = Text
                            print(Style.RESET_ALL, end='')
                            continue
                        ...
                        for fallback_index, fallback_url in enumerate(fallback_urls):
                            ...
                            try:
                                ...
                                item_list.append(
                                    ['communique-de-presse', fallback_url, pr_object["title"], content,
                                     0, 'Banque de France', file_helper.try_parse_date(pr_object["title"], year_object["year"])])  # ContentType: 0 = Text
                                break
                            except JavascriptException as jve:
