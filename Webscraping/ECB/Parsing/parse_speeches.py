...
def parse(record_id, _author, _post_link, _link_type, _published_date, _section):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            #content_section = soup.select('div.section > p')
            content_section = soup.select('article > p, article > ul, article > ol > li')
            #content_section = soup.select('div.itemizedlist > ul > li')
            #content_section = soup.select('div.orderedlist > ol > li')
            #content_section = soup.select('main > p')
