...
Interventions <- read.csv("~/Master_Interventions_08262021.csv")#214 Items in Folder

Discourse <- read.csv("~/Master_Discourse_08282021.csv") #239 Items in Folder

Interviews_BB <- read.csv("~/Master_Interviews_BB_08282021.csv") #588 Items in Folder

Interviews_BF <- read.csv("~/Master_Interventions_08262021.csv") #267 Items in Folder

Interviews_ECB <- read.csv("~/Master_Interviews_ECB_08282021.csv") # 338 Items in Folder

Press_ECB <- read.csv("~/Master_Press_ECB_08282021.csv") # 453 Items in Folder

Reden <- read.csv("~/Master_08312021.csv") # 1098 Items in Folder

All_ECB <- read.table("~/all_ECB_speeches.csv", sep = "|", quote = "", fill = TRUE, header = TRUE, encoding = "UTF-8", stringsAsFactors = FALSE)

Speakers_2 <- read.csv("~/Moritz_Speakers_08302021.csv")

Non_Words <- read.table(file = "~/Stopwords/Selected_Stopwords.txt")

Bi_Grams <- read.csv("~/Dict_ENG\\Dict_Eng_2/Insiders_Eng_2.csv")

Out_Dicts <- read.csv("~/Dict_ENG\\Dict_Eng_2/Outsiders_Eng_2.csv")

Super_Dicts <- read.csv("~/Dict_ENG/Dict_Eng_2/Super_Insiders_Eng_2.csv")

Trust_Dicts <- read.csv("~/Dict_ENG/Dict_Eng_2/Trust_Eng_2.csv")
...
All_Speeches_2 <- subset(All_Speeches_2, !All_Speeches_2$AUTHOR_2=="ALEXANDRE LAMFALUSSY"&
                           ...
                           !All_Speeches_2$AUTHOR_2=="SIRKKA H?M?L?INEN"&
...
##
## Extracting Source data to merge on modeling script
##

#Source_Data <- Final_Comms[,c("Speech_ID","SOURCE", "Total_Day_Comm")]
#write.csv(Source_Data, file = "Source_Data.csv")
...
Final_Comms$TEXT_Non_Stop <- stemDocument(Final_Comms$TEXT_Non_Stop, language = "english")
?stemDocument
...
Bi_Grams$N_Grams <- stemDocument(Bi_Grams$N_Grams, language = "english")
...
Out_Dicts$N_Grams <- stemDocument(Out_Dicts$N_Grams, language = "english")
...
Super_Dicts$N_Grams <- stemDocument(Super_Dicts$N_Grams, language = "english")
...
Trust_Dicts$N_Grams <- stemDocument(Trust_Dicts$N_Grams, language = "english")
...
Eng_Counts <- subset(Final_Comms, Final_Comms$LANGUAGE == 'english')
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:2341], sum))
...
#write.csv(Non_Sense_Insiders, file = "Non_Sense_Insiders.csv")
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:1798], sum))
...
#write.csv(Non_Sense_Insiders, file = "Non_Sense_SuperInsiders_2.csv")
...
Tone_Super <- as.data.frame(Eng_Super_Insiders$Tone)
#write.csv(Tone_Super, file = "Tone_Super.csv")
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:483], sum))
...
#write.csv(Non_Sense_Insiders, file = "Non_Sense_Outsiders.csv")
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:235], sum))
...
#write.csv(Non_Sense_Insiders, file = "Non_Sense_Trust.csv")
...
Total_Eng <- Total_Counts[,c("Speech_ID","Tone_Insiders", "Tone_Super_Insiders", "Tone_Outsiders", "Tone_Trust")]
#write.csv(Total_Eng, file = "Total_Eng.csv")
...
#write.csv(Eng_Counts, file = "Eng_Counts_David.csv")
#write.csv(Out_Dicts, file = "Out_Dicts_David.csv")
#write.csv(Bi_Grams, file = "Bi_Grams_David.csv")

Eng_Counts$Total_Negative_Insiders <- rowSums(Eng_Counts[,3:1177])

Eng_Counts$Total_Positive_Insiders <- rowSums(Eng_Counts[,1178:2082])

Eng_Counts_2 <- Eng_Counts[,c("Speech_ID", "TEXT_Non_Stop", "Total_Positive", "Total_Negative")]

Last_Negative <- Eng_Counts[,1176:1179]


Test_1 <- subset(Eng_Counts, Eng_Counts$Speech_ID== "2019-12-18_BENO?T COUR?")


Test_1 <- Test_1[,c("Speech_ID","declinwage")]


Eng_Counts_2$Numerator <- Eng_Counts_2$Total_Positive - Eng_Counts_2$Total_Negative
Eng_Counts_2$Denominator <- Eng_Counts_2$Total_Positive + Eng_Counts_2$Total_Negative

Eng_Counts_2$Tone <- Eng_Counts_2$Numerator/Eng_Counts_2$Denominator

hist(Eng_Counts_2$Tone, breaks = 30)

plot(density(Eng_Counts_2$Tone))


Eng_Counts_2$Flag <- ifelse(Eng_Counts_2$Total_Positive==0 & Eng_Counts_2$Total_Negative ==1,1,0)
Eng_Counts_2$Flag_2 <- ifelse(Eng_Counts_2$Total_Positive==1 & Eng_Counts_2$Total_Negative ==0,1,0)

sum(Eng_Counts_2$Flag)
sum(Eng_Counts_2$Flag_2)


sum(ifelse( is.nan(Eng_Counts_2$Tone)== TRUE,1,0))

lapply(Last_Negative, sum)

rm(Eng_Counts)
class(Eng_Counts)

sum(as.numeric(Eng_Counts$Economi))
Eng_Counts[1,2]

Eng_Corpus <- SimpleCorpus(VectorSource(Eng_Counts))

DTM <- DocumentTermMatrix(Eng_Counts)


##
## Investigate string frequency Corpus tm package
##

En_Sp <- Eng_Counts[1,c("Speech_ID", "TEXT_Non_Stop")]

Speeches <- SimpleCorpus(VectorSource(En_Sp$TEXT_Non_Stop))

#Speeches_2 <- tm_map(Speeches, stripWhitespace)

Speeches_2 <- tm_map(Speeches, removeNumbers)

#Speeches_2 <- tm_map(Speeches_2, removePunctuation)

#Speeches_2 <- tm_map(Speeches_2, stemDocument)

#Speeches_2 <- tm_map(Speeches_2, removeWords, stopwords("english"))

Speeches_2[[1]]$content





DTM <- DocumentTermMatrix(Speeches_2)
inspect(DTM)
Word_Frequency <- as.data.frame(colSums(as.matrix(DTM)))
...
Non_Words_2 <- Final_Comms[10:20,c("TEXT_2","TEXT_Non_Stop")]

print(Non_Words_2$TEXT_2)
print(Non_Words_2$TEXT_Non_Stop)

Test_1 <- subset(All_Speeches_3, rownames(All_Speeches_3)== c(147,151))
class(Test_1)
?preprocess

Test_1$TEXT_2 <- tolower(Test_1$TEXT)
Test_1$TEXT_3 <- gsub('[[:punct:] ]+',' ',Test_1$TEXT_2)

Test_2 <- Test_1[,c("TEXT", "TEXT_2", "TEXT_3")]

class(Test_2)


All_Speeches_3$TEXT_2 <- tolower(All_Speeches_3$TEXT)
All_Speeches_3$TEXT_2 <- gsub('[[:punct:] ]+',' ',All_Speeches_3$TEXT_2)
class(All_Speeches_3)



#write.csv(Final_Comms, file = "Final_Comms_09012021.csv")


Test_3 <- subset(All_Speeches_3, rownames(All_Speeches_3)==212)
Test_3 <- Test_3[,c("TEXT")]
rawToChar(Test_3$TEXT)


TEXT_2 <- lapply(Test_1$TEXT, preprocess)
TEXT_2 <- as.data.frame(TEXT_2)

Test_1$TEXT <- as.list(Test_1$TEXT)
Test_1$TEXT
class(All_Speeches_3)

Test_1 <- as.data.frame(Test_1$TEXT, Test_1$Text_2)
wordcount(as.data.frame(Test_1$Text_2), count.function = sum)

Test_1 <- Test_1[,c("TEXT", "Text_2")]
All_Speeches_3[2,4]

unique(ngram::wordcount(All_Speeches_3$TEXT))

stopwords(language = "en")
?stopwords_getlanguages()

All_Speeches_3[1,4]
Test_1 <- as.data.frame(All_Speeches_3[1,4])
preprocess(Test_1$TEXT,case = "lower", remove.punct = TRUE, remove.numbers = TRUE, fix.spacing = TRUE)
?preprocess
...
colnames(Non_Words)[1] <- c("List_Words")


Non_Words <- tm_map(as.list(Non_Words),removeWords,c(as.list(Non_Words)))


str_detect(Test_5$TEXT_2, "payments")
En_Sp$Word_Count <- str_count(En_Sp$TEXT_2)


Test_1 <- En_Sp[1:3,c("Speech_ID", "TEXT_2")]


Test_1[3,2]


wordcloud(words = head$terms, freq = head$count, min.freq = 1000, max.words = 500)


rm(head, Speeches, Speeches_2, sums, Other_Languages, DTM ,Test_1)
