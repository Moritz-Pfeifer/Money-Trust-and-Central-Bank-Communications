...
Non_Words <- read.table(file = "~/stopwords-fr.txt")

Bi_Grams <- read.csv("~/Dict_Insiders_FR_2.csv")

Out_Dicts <- read.csv("~/Dict_Outsider_FR_2.csv")

Super_Dicts <- read.csv("~/Dict_Super_FR_2.csv")

Trust_Dicts <- read.csv("~/Dict_Trust_FR_2.csv")
...
#Speaker_List <- read.csv("~/Speakers_List.csv")
...
All_Speeches_3 <- subset(All_Speeches_3, All_Speeches_3$LANGUAGE == "french")
...
Final_Comms$TEXT_Non_Stop <- stemDocument(Final_Comms$TEXT_Non_Stop, language = "french")
?stemDocument
...
##
## Counting French Dictionary
##

#
# processing french dictionary n grams
#
...
Out_Dicts$N_Grams <- stemDocument(Out_Dicts$N_Grams, language = "french")
Out_Dicts$N_Grams_2 <- stemDocument(Out_Dicts$N_Grams_2, language = "french")
...
Super_Dicts$N_Grams <- stemDocument(Super_Dicts$N_Grams, language = "french")
Super_Dicts$N_Grams_2 <- stemDocument(Super_Dicts$N_Grams_2, language = "french")
...
Trust_Dicts$N_Grams <- stemDocument(Trust_Dicts$N_Grams, language = "french")
Trust_Dicts$N_Grams_2 <- stemDocument(Trust_Dicts$N_Grams_2, language = "french")
...
Eng_Counts <- subset(Final_Comms, Final_Comms$LANGUAGE == 'french')
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:4386], sum))
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:3574], sum))
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:1110], sum))
...
Non_Sense_Insiders <- as.data.frame(lapply(Eng_Counts[,3:544], sum))
...
Total_Fr <- Total_Counts[,c("Speech_ID","Tone_Insiders","Tone_Super_Insiders", "Tone_Trust", "Tone_Outsiders")]

#write.csv(Total_Fr, file = "Total_Fr.csv")
...
Bi_Grams_Words <- as.data.frame(unique(Bi_Grams$Adjetives_Verbs))
colnames(Bi_Grams_Words)[1]<- c("Key")
Bi_Grams_Words_2 <- as.data.frame(unique(Bi_Grams$Nouns_Dependent))
colnames(Bi_Grams_Words_2)[1]<- c("Key")
Bi_Grams_Words_3 <- as.data.frame(unique(Super_Dicts$Adjetives_Verbs))
colnames(Bi_Grams_Words_3)[1]<- c("Key")
Bi_Grams_Words_4 <- as.data.frame(unique(Super_Dicts$Nouns_Dependent))
colnames(Bi_Grams_Words_4)[1]<- c("Key")
Bi_Grams_Words_5 <- as.data.frame(unique(Out_Dicts$Adjetives_Verbs))
colnames(Bi_Grams_Words_5)[1]<- c("Key")
Bi_Grams_Words_6 <- as.data.frame(unique(Out_Dicts$Nouns_Dependent))
colnames(Bi_Grams_Words_6)[1]<- c("Key")
Bi_Grams_Words_7 <- as.data.frame(unique(Trust_Dicts$Adjetives_Verbs))
colnames(Bi_Grams_Words_7)[1]<- c("Key")
Bi_Grams_Words_8 <- as.data.frame(unique(Trust_Dicts$Nouns_Dependent))
colnames(Bi_Grams_Words_8)[1]<- c("Key")


All_Words <- as.data.frame(rbind(Bi_Grams_Words,
                                 Bi_Grams_Words_2,
                                 Bi_Grams_Words_3,
                                 Bi_Grams_Words_4,
                                 Bi_Grams_Words_5,
...
All_Words$Stemmed_Key <- stemDocument(All_Words$Key, language = "french")

Eng_Out_TDM <- Corpus(VectorSource(Eng_Outsiders$TEXT_Non_Stop))
Eng_Out_TDM <-  as.matrix(TermDocumentMatrix(Eng_Out_TDM)) 
Eng_Out_TDM <- data.frame(N_Grams=rownames(Eng_Out_TDM),
                          Freq = rowSums(Eng_Out_TDM),
                          row.names = NULL)

Eng_Out_TDM$Match <- match(Eng_Out_TDM$N_Grams,All_Words$Stemmed_Key)
table(Eng_Out_TDM$Match)
Eng_Out_TDM <- Eng_Out_TDM[order(Eng_Out_TDM$Match, decreasing = TRUE),]
